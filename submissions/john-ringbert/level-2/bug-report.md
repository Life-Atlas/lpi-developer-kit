Bug-report
What I tested
I ran the LPI sandbox locally and tested it with empty inputs, whitespace-only inputs, invalid values, special characters, emoji, long queries and also some stress testing.
What worked
...
A query with only spaces returned results instead of being treated as empty input. In get_insights, both an empty tier and an invalid value like banana still returned normal output.
